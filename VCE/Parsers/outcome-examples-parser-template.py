...
def load_config(config_path: str) -> Dict:
    ...
    # No auto-detection — config must explicitly declare sample_teaching.
    # This prevents sub-subject configs from picking up a shared parent's STL doc.
    sample_teaching = config.get('documents', {}).get('sample_teaching', '')
    if sample_teaching:
        config['examples_doc'] = str(docs_dir / sample_teaching)
    else:
        config['examples_doc'] = ''

    # Resolve output directory (same as curriculum parser - Curriculum folder)
    config['output_dir'] = str(subject_dir / "graphRAG_csv" / "Curriculum")

    # Resolve outcomes CSV path (generated by curriculum parser)
    slug = config.get('subject_slug', config.get('subject', '').lower().replace(' ', '_'))
    config['outcomes_csv'] = str(subject_dir / "graphRAG_csv" / "Curriculum" /
                                  f"vcaa_vce_sd_{slug}_curriculum-outcomes.csv")
